...
CAUSE_RULES = [
    ...
    (
        "long_shot",
        lambda r, ctx: r["shot_age"] >= max(8.0, ctx["shot_age_p80"]),
        "the same shot has been held for {shot_age:.0f}s with no cut",
    ),
    (
        "slow_cutting",
        lambda r, ctx: r["cut_rate"] <= ctx["cut_rate_p20"] and r["cut_rate"] < 1.0,
        "cutting has slowed to {cut_rate:.1f} cuts per 10s",
    ),
    (
        "dead_air",
        lambda r, ctx: r["silence"] >= 0.45,
        "{silence:.0%} of the window has no speech",
    ),
    (
        "slow_speech",
        lambda r, ctx: r["speech_rate"] <= ctx["speech_rate_p20"],
        "delivery has dropped to {speech_rate:.1f} words/s",
    ),
    (
        "mix_drop",
        lambda r, ctx: r["loudness_delta"] <= -4.0,
